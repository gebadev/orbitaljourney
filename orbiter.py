...
class Orbiter:
    ...
    def update(self, planets):
        if self.in_orbit:
            ...
            self.x = (
                self.orbiting_planet.x
                + self.orbiting_planet.orbit_radius * math.cos(self.angle)
            )
            self.y = (
                self.orbiting_planet.y
                + self.orbiting_planet.orbit_radius * math.sin(self.angle)
            )
        else:
            self.x += self.vx
            self.y += self.vy
            ...
            for planet in planets:
                if planet != self.orbiting_planet:
                    distance = math.sqrt(
                        (self.x - planet.x) ** 2 + (self.y - planet.y) ** 2
                    )
                    if (
                        distance <= planet.orbit_radius
                        and distance > planet.radius
                    ):
    ...
    def leave_orbit(self):
        if self.in_orbit:
            ...
            self.vx = (
                -self.orbiting_planet.orbit_radius
                * math.sin(self.angle)
                * ORBITAL_SPEED
