...
class Game:
        ...
        for _ in range(num_planets):
            ...
            while attempts < 50:
                ...
                for existing_planet in self.planets:
                    distance = math.sqrt(
                        (x - existing_planet.x) ** 2 + (y - existing_planet.y) ** 2
                    )
    ...
    def update(self):
        if self.game_state == "title":
            ...
            for planet in self.planets:
                distance = math.sqrt(
                    (self.orbiter.x - planet.x) ** 2 + (self.orbiter.y - planet.y) ** 2
                )
            ...
            if (
                self.orbiter.x <= 0
                or self.orbiter.x >= SCREEN_WIDTH
                or self.orbiter.y <= 0
                or self.orbiter.y >= SCREEN_HEIGHT
            ):
                self.game_over = True
    ...
    def draw(self):
        ...
        if self.game_state == "title":
            # Title screen
            pyxel.text(100, SCREEN_HEIGHT // 2 - 40, GAME_TITLE, 9)
            pyxel.text(99, SCREEN_HEIGHT // 2 - 40, GAME_TITLE, 10)
            pyxel.text(
                80,
                100,
                "Press SPACE to leave orbit.",
                7,
            )
            pyxel.text(
                80,
